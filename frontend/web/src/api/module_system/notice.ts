...
const NoticeAPI = {
  async listNotice(query: NoticePageQuery) {
    let builder = baas.database.from("sys_notice").select("*", { count: "exact" });
  ...
  async listNoticeAvailable() {
    const rows =
      (unwrap(await baas.database.from("sys_notice").select("*").eq("status", 1)) as NoticeTable[]) || [];
    return ok(rows);
  },

  async detailNotice(id: number) {
    const rows = unwrap(await baas.database.from("sys_notice").select("*").eq("id", id)) as NoticeTable[];
  ...
  async createNotice(body: NoticeForm) {
    unwrap(
      await baas.database.from("sys_notice").insert([
        {
  ...
  async deleteNotice(body: number[]) {
    unwrap(await baas.database.from("sys_notice").delete().in("id", body));
    return ok(null, "删除成功", true);
  },

  async batchNotice(body: BatchType) {
    unwrap(await baas.database.from("sys_notice").update({ status: body.status }).in("id", body.ids));
    return ok(null, "更新成功", true);
  },
};
